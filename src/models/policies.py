...
def select_vlm_hidden_states(vlm_hidden_states, num_blocks, layer_indices=None):
    if layer_indices is None:
        return vlm_hidden_states[-num_blocks:]
    if len(layer_indices) != num_blocks:
        raise ValueError(
            f"vlm_layer_indices length ({len(layer_indices)}) must match action expert depth ({num_blocks})."
        )
    return tuple(vlm_hidden_states[int(i)] for i in layer_indices)
...
class ActionDiffusionTransformerMoE(nn.Module):
    def __init__(self, action_dim, vlm_hidden_size, hidden_size=384, depth=12, num_heads=6, mlp_ratio=4.0, gen_hidden_size=None, vlm_layer_indices=None):
        super().__init__()
    ...
    def forward(self, noisy_action, timestep, vlm_hidden_states, history_actions=None, gen_hidden_states=None):
        ...
        relevant_vlm_states = select_vlm_hidden_states(vlm_hidden_states, len(self.blocks), self.vlm_layer_indices)
...
class DualFlowActionDiffusionTransformerMoE(nn.Module):
    def __init__(
        self,
        action_dim,
        vlm_hidden_size,
        ...
        self.cross_norm_a = nn.ModuleList([
            nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
            for _ in range(depth)
        ])
        self.cross_norm_v = nn.ModuleList([
            nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
            for _ in range(depth)
        ])
        self.cross_av = nn.ModuleList([
            nn.MultiheadAttention(hidden_size, num_heads, batch_first=True)
            for _ in range(depth)
        ])
        self.cross_va = nn.ModuleList([
            nn.MultiheadAttention(hidden_size, num_heads, batch_first=True)
            for _ in range(depth)
        ])
        self.gamma_av = nn.Parameter(torch.zeros(depth))
        self.gamma_va = nn.Parameter(torch.zeros(depth))
        self.final_layer_a = FinalLayer1D(hidden_size, action_dim)
        self.final_layer_v = FinalLayer1D(hidden_size, action_dim)
        self.initialize_weights()
    ...
    def initialize_weights(self):
        ...
        for block in self.blocks:
            ...
            nn.init.constant_(block.adaLN_modulation[-1].weight, 0)
            nn.init.constant_(block.adaLN_modulation[-1].bias, 0)
        for final_layer in (self.final_layer_a, self.final_layer_v):
    ...
    def forward(self, action_latent, visual_latent, timestep, vlm_hidden_states, gen_hidden_states=None):
        action_latent = action_latent.to(dtype=self.input_proj.weight.dtype)
        visual_latent = visual_latent.to(dtype=self.input_proj.weight.dtype)
        x_a = self.input_proj(action_latent)
        x_v = self.input_proj(visual_latent)
        x_a = x_a + self.pos_embed[:, :x_a.shape[1], :]
        x_v = x_v + self.pos_embed[:, :x_v.shape[1], :]
        t = self.t_embedder(timestep)
        ...
        for i, (block, vlm_state, gen_state) in enumerate(zip(self.blocks, relevant_vlm_states, relevant_gen_states)):
            vlm_state = vlm_state.to(dtype=x_a.dtype)
            if gen_state is not None:
                gen_state = gen_state.to(dtype=x_a.dtype)
            x_a = block(x_a, t, vlm_state, gen_feat=gen_state)
            x_v = block(x_v, t, vlm_state, gen_feat=gen_state)

            a_q = self.cross_norm_a[i](x_a)
            v_q = self.cross_norm_v[i](x_v)
            delta_a, _ = self.cross_av[i](query=a_q, key=v_q, value=v_q, need_weights=False)
            delta_v, _ = self.cross_va[i](query=v_q, key=a_q, value=a_q, need_weights=False)
            x_a = x_a + self.gamma_av[i].to(dtype=x_a.dtype) * delta_a
            x_v = x_v + self.gamma_va[i].to(dtype=x_v.dtype) * delta_v

        velocity_a = self.final_layer_a(x_a, t)
        velocity_v = self.final_layer_v(x_v, t)
        return velocity_a, velocity_v
...
class ActionRegressionTransformerMoE(nn.Module):
    def __init__(self, action_dim, vlm_hidden_size, num_actions=1, hidden_size=384, depth=12, num_heads=6, mlp_ratio=4.0, gen_hidden_size=None, vlm_layer_indices=None):
        super().__init__()
        self.vlm_layer_indices = tuple(vlm_layer_indices) if vlm_layer_indices is not None else None
        self.num_actions = num_actions
        self.action_dim = action_dim
...
class ActionClassificationTransformerMoE(nn.Module):
    def __init__(self, action_dim, vlm_hidden_size, num_actions=1, num_bins=256,
                 hidden_size=384, depth=12, num_heads=6, mlp_ratio=4.0,
                 vqvae_mode=False, vq_codebook_size=1024, vq_latent_codes=3, gen_hidden_size=None, vlm_layer_indices=None):
        super().__init__()
        self.vlm_layer_indices = tuple(vlm_layer_indices) if vlm_layer_indices is not None else None
        self.num_actions = num_actions
        self.action_dim = action_dim
